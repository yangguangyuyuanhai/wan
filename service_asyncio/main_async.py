...
from scheduler_async import AsyncPipelineScheduler
from logger_config import get_logger

logger = get_logger("AsyncMain")


class AsyncVisionSystem:
    """
    异步视觉系统上帝类
    负责整个系统的生命周期管理
    支持多相机并发处理
    ...
    async def run(self):
        ...
        try:
            # 创建调度器
            self.scheduler = AsyncPipelineScheduler(self.config)
...
示例:
  ...
  # 自定义参数
  python main_async.py --exposure 15000 --gain 12.0 --confidence 0.6
